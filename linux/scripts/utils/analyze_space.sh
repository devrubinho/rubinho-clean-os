...
find_cleanable_files() {
    ...
    if [ "$EUID" -eq 0 ]; then
        ...
        done < <(sudo find /home -type f -name "$pattern" 2>/dev/null)
    else
        while IFS= read -r path; do
            if [ -f "$path" ]; then
                local size_kb=$(du -sk "$path" 2>/dev/null | cut -f1)
        ...
        done < <(find /home -type f -name "$pattern" 2>/dev/null)
    fi
}
        ...
        for user_dir in /home/*; do
            if [ -d "$user_dir" ]; then
                if [ -d "$user_dir/.cache" ]; then
                    local size_kb
                    if [ "$EUID" -eq 0 ]; then
                        size_kb=$(sudo du -sk "$user_dir/.cache" 2>/dev/null | cut -f1)
                        size_human=$(sudo du -sh "$user_dir/.cache" 2>/dev/null | awk '{print $1}')
                    else
                        size_kb=$(du -sk "$user_dir/.cache" 2>/dev/null | cut -f1)
                        size_human=$(du -sh "$user_dir/.cache" 2>/dev/null | awk '{print $1}')
                    fi
                    if [ -n "$size_kb" ] && [ "$size_kb" -gt 0 ]; then
                        echo "$size_kb $size_human $user_dir/.cache"
                    fi
                fi
            fi
        done >> "$CLEANABLE_TEMP"
        ...
        for user_dir in /home/*; do
            if [ -d "$user_dir" ]; then
                if [ -d "$user_dir/.local/share/Trash" ]; then
                    local size_kb
                    if [ "$EUID" -eq 0 ]; then
                        size_kb=$(sudo du -sk "$user_dir/.local/share/Trash" 2>/dev/null | cut -f1)
                        size_human=$(sudo du -sh "$user_dir/.local/share/Trash" 2>/dev/null | awk '{print $1}')
                    else
                        size_kb=$(du -sk "$user_dir/.local/share/Trash" 2>/dev/null | cut -f1)
                        size_human=$(du -sh "$user_dir/.local/share/Trash" 2>/dev/null | awk '{print $1}')
                    fi
                    if [ -n "$size_kb" ] && [ "$size_kb" -gt 0 ]; then
                        echo "$size_kb $size_human $user_dir/.local/share/Trash"
                    fi
                fi
            fi
        done >> "$CLEANABLE_TEMP"
...
if [ -s "$CLEANABLE_TEMP" ]; then
    ...
    awk '{
        ...
        # e.g., .cache, .local/share/Trash, vendor/bundle
        if (path ~ /\/\.cache$/) {
            item_name = ".cache"
